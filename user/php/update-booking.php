...
if (empty($_POST["ba-date-time"])) {
	...
	if ($date <= $newDateTime) {
		...
		if ($date != $booking_date_time) {
			$sql1 = "UPDATE booking SET booking_title = '$fba_tittle' , booking_comment = '$fba_comment', booking_date_time = '$date', booking_status = 'Changing' WHERE user_ID = '$user_id' AND booking_ID = '$booking_id ' ";
			...
			$sql3 = "INSERT into inbox(tech_ID, inbox_message, created_at, inbox_read) values('$tech_ID', 'A booking date has update Appointment ID: $booking_id, You has option to accept it or cancel it', '$current_date', '0')";
